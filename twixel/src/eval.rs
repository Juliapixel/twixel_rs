use std::{future::IntoFuture, sync::{atomic::{AtomicBool, Ordering}, Arc, LazyLock}};

use futures::{future::OptionFuture, FutureExt, TryFutureExt};
use reqwest::header::{HeaderMap, ACCEPT, USER_AGENT};
use rquickjs::{function::Async as AsyncJsClosure, prelude::{Func, Opt}, promise::PromiseFuture, Array, AsyncContext, AsyncRuntime, Coerced, Ctx, FromJs, IntoJs, IteratorJs, Object, String as JsString, Type, Value};
use tokio::task::LocalSet;
use twixel_core::irc_message::tags::OwnedTag;

use crate::{bot::{Bot, BotCommand}, command::{CommandContext, CommandHandler}};
    ...
    std::thread::spawn(move || {
        tokio::runtime::Builder::new_current_thread()
            ...
            .block_on(async move {
                ...
                // rt.set_interrupt_handler(Some(Box::new(move || start_time.elapsed().as_secs() > 5))).await;
            ...
            match val {
                Ok(val) => {
                    match val.type_of() {
                        ...
                        | Type::Module
                        | Type::Unknown => {
                            ctx.json_stringify(val).and_then(|i| i.map(|s| s.to_string()).unwrap()).expect("aaaa")
                        },
                    }
                },
...
impl<'js> FromJs<'js> for JsRequestInit {
    fn from_js(ctx: &rquickjs::Ctx<'js>, value: Value<'js>) -> rquickjs::Result<Self> {
        ...
        let obj = value.try_into_object().map_err(|_| rquickjs::Error::new_from_js(type_name, "object"))?;

        let body = String::from_js(ctx, obj.get("body")?).ok();
        let method = String::from_js(ctx, obj.get("method")?)?.parse().unwrap_or_default();
...
impl<'js> IntoJs<'js> for ValueJs {
    fn into_js(self, ctx: &Ctx<'js>) -> rquickjs::Result<Value<'js>> {
        match self.0 {
            ...
            serde_json::Value::Array(vec) => vec.into_iter().map(ValueJs).collect_js::<Array<'js>>(ctx).map(|a| a.into_value()),
...
async fn fetch_inner(url: JsString<'_>, opts: JsRequestInit) -> Result<Object<'_>, FetchError> {
    ...
    let resp = JS_CLIENT.execute(JS_CLIENT.request(opts.method, url).build()?).await?;
